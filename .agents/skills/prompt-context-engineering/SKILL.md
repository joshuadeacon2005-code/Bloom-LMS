...
Design high-signal, low-latency prompts and context contracts for production agent tasks. Use when writing system prompts, skill prompts, subagent prompts, or rewrite instructions where consistency is required on first pass.

| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `phrazzld-agent-skills-prompt-context-engineering`                                |
| Version    | 1.0.1                                     |
| Author     | phrazzld                                 |
| Category   | ai-llms                                    |
| Installs   |                                 |
...
**GitHub:** [phrazzld/agent-skills](https://github.com/phrazzld/agent-skills) — ⭐ 1 | Forks: 
...
Prompt + Context Engineering (Latency-First) is a prescriptive skill for designing compact, deterministic system and subagent prompts that maximize first-pass correctness without multi-pass retries. It codifies core features: role+objective+quality-bar framing, hard output contracts (required fields, allowed transformations, forbidden behaviors), concise context-as-signal metadata, and a deterministic posture (low temperature, stable instructions). Use cases include authoring system prompts, skill/subagent prompts, rewrite instructions, and CI-aligned evaluation prompts for production agents, low-latency pipelines, and streaming inference. Built-in patterns and a pre-shipping checklist prevent drift and anti-patterns while producing machine-verifiable output contracts. Invocations return a standardized report (Diagnosis, Prompt Delta, Context Delta, First-Pass Reliability Risks, Suggested Eval Assertions) to guide safe, measurable deployment and automated testing.
...
```bash
npx -y @lobehub/market-cli skills install phrazzld-agent-skills-prompt-context-engineering
```
...
npx -y @lobehub/market-cli skills install phrazzld-agent-skills-prompt-context-engineering --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install phrazzld-agent-skills-prompt-context-engineering --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install phrazzld-agent-skills-prompt-context-engineering --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install phrazzld-agent-skills-prompt-context-engineering --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install phrazzld-agent-skills-prompt-context-engineering --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate phrazzld-agent-skills-prompt-context-engineering --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment phrazzld-agent-skills-prompt-context-engineering -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment phrazzld-agent-skills-prompt-context-engineering
```
...
```bash
npx -y @lobehub/market-cli skills comments phrazzld-agent-skills-prompt-context-engineering
```
