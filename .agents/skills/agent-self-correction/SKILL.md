...
AI agent self-correction mechanisms enable agents to detect errors, validate outputs, and automatically recover from failures. This includes validation loops, confidence scoring, iterative refinement,

| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `amnadtaowsoam-cerebraskills-agent-self-correction`                                |
| Version    | 1.0.0                                     |
...
**GitHub:** [AmnadTaowsoam/CerebraSkills](https://github.com/AmnadTaowsoam/CerebraSkills) — ⭐ 1 | Forks: 
...
Agent Self Correction provides a systematic framework for building reliable AI agents by combining error detectors, validation loops, confidence scoring, iterative refinement, and recovery strategies. Key features include pluggable error detection modules, configurable validation rules and thresholds, automated re-prompting or repair actions, a self-reflection subsystem that diagnoses failure modes, and telemetry for confidence trends and improvement metrics. Use cases include production chatbots, autonomous workflow agents, backend data pipelines, and any system that requires automated output verification and recovery from LLM failures. Core advantages are reduced human intervention, higher output quality through convergence of validation loops, measurable trust via confidence scores, and faster incident resolution. Integrates with LLM APIs and provides artifacts such as detector implementations, recovery playbooks, and acceptance evidence for deployment.
...
```bash
npx -y @lobehub/market-cli skills install amnadtaowsoam-cerebraskills-agent-self-correction
```
...
npx -y @lobehub/market-cli skills install amnadtaowsoam-cerebraskills-agent-self-correction --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install amnadtaowsoam-cerebraskills-agent-self-correction --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install amnadtaowsoam-cerebraskills-agent-self-correction --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install amnadtaowsoam-cerebraskills-agent-self-correction --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install amnadtaowsoam-cerebraskills-agent-self-correction --version 1.0.0
```
...
```bash
npx -y @lobehub/market-cli skills rate amnadtaowsoam-cerebraskills-agent-self-correction --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment amnadtaowsoam-cerebraskills-agent-self-correction -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment amnadtaowsoam-cerebraskills-agent-self-correction
```
...
```bash
npx -y @lobehub/market-cli skills comments amnadtaowsoam-cerebraskills-agent-self-correction
```
