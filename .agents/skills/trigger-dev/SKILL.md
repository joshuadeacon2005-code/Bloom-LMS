...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `terminalskills-skills-trigger-dev`                                |
| Version    | 1.0.1                                     |
...
Trigger.dev v3 is a developer-focused platform for running long-running background jobs from serverless environments. It lets you define tasks in your codebase (via an SDK), trigger them from API routes or webhooks, and run them reliably on managed infrastructure with no platform timeouts. Key features include built-in retries with backoff, concurrency control, scheduling, and full observability (logs, traces, and run history). Common use cases are AI/LLM pipelines, video or large-file processing, robust webhook handling, async job queues, and scheduled jobs — effectively replacing self-hosted systems like BullMQ or Celery in serverless apps. Integrations and examples cover Next.js, Remix, and Express. Developer conveniences include a CLI for init/login/dev mode and an SDK for type-safe task definitions and payload handling.
...
```bash
npx -y @lobehub/market-cli skills install terminalskills-skills-trigger-dev
```
...
npx -y @lobehub/market-cli skills install terminalskills-skills-trigger-dev --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install terminalskills-skills-trigger-dev --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install terminalskills-skills-trigger-dev --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install terminalskills-skills-trigger-dev --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install terminalskills-skills-trigger-dev --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate terminalskills-skills-trigger-dev --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment terminalskills-skills-trigger-dev -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment terminalskills-skills-trigger-dev
```
...
```bash
npx -y @lobehub/market-cli skills comments terminalskills-skills-trigger-dev
```
