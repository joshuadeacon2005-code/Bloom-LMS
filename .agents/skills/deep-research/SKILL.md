...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `petekp-agent-skills-deep-research`                                |
| Version    | 1.0.1                                     |
| Author     | petekp                                 |
...
**GitHub:** [petekp/agent-skills](https://github.com/petekp/agent-skills) — ⭐  | Forks: 
...
Deep Research is a systematic, citation-first research Skill designed for exhaustive, high-confidence investigations. It enforces core principles—multiple independent sources, mandatory citations, primary-source preference, explicit uncertainty, and cross-referencing—and orchestrates a toolbox that includes WebSearch, WebFetch, Playwright browser, documentation indexes, and local code grep. Use it for fact-checking, technology or API deep-dives, competitive comparisons, current-events verification, and codebase or documentation discovery where accuracy and traceability matter. The workflow begins with scope definition (questions, depth, recency, authoritative benchmarks), proceeds to broad source discovery and targeted retrieval, and ends with reconciled findings, conflict notes, confidence scores, and source-quality assessments. Outputs are structured, citation-rich reports that prioritize correctness over speed and ask clarifying questions when scope is ambiguous.
...
```bash
npx -y @lobehub/market-cli skills install petekp-agent-skills-deep-research
```
...
npx -y @lobehub/market-cli skills install petekp-agent-skills-deep-research --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install petekp-agent-skills-deep-research --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install petekp-agent-skills-deep-research --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install petekp-agent-skills-deep-research --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install petekp-agent-skills-deep-research --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate petekp-agent-skills-deep-research --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment petekp-agent-skills-deep-research -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment petekp-agent-skills-deep-research
```
...
```bash
npx -y @lobehub/market-cli skills comments petekp-agent-skills-deep-research
```
